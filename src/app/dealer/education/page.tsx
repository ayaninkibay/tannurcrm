...
export default function EducationContent() {
  ...
  // маппинг категорий -> id курса на превью (all_courses)
  const categoryIdMap: Record<string, number> = {
    'Знакомство с Tannur': 1,
    'Маркетинговая стратегия': 2,
    // остальные пока без реальных id — пойдём по title (заглушка)
  };

  const getCategoryHref = (title: string) => {
    const id = categoryIdMap[title];
    return id
      ? `/dealer/education/all_courses?id=${id}`
      : `/dealer/education/all_courses?title=${encodeURIComponent(title)}`;
  };

  const categories = [
    { title: 'Знакомство с Tannur', count: 3, icon: <Sparkles className="w-5 h-5" /> },
    { title: 'Маркетинговая стратегия', count: 7, icon: <TrendingUp className="w-5 h-5" /> },
    { title: 'Менеджер по продажам', count: 3, icon: <Users className="w-5 h-5" /> },
    { title: 'Как продавать продукцию?', count: 3, icon: <ShoppingBag className="w-5 h-5" /> }
  ];

  const courses = [
    { id: 1, image: '/icons/edu_1.png', category: 'Знакомство', title: 'Знакомство с продуктами Tannur', lessons: 8, duration: '2 часа', isNew: true },
    { id: 2, image: '/icons/edu_2.png', category: 'Маркетинг', title: 'Маркетинговая стратегия 2.0', lessons: 12, duration: '3.5 часа', isPopular: true },
    { id: 3, image: '/icons/edu_3.png', category: 'Продукты', title: 'Уходовый набор вместе с Інжу Ануарбек', lessons: 6, duration: '1.5 часа' },
    { id: 4, image: '/icons/edu_4.png', category: 'Практика', title: 'Эффективные продажи в сети', lessons: 10, duration: '4 часа' }
  ];
    ...
    <div className="p-2 md:p-6">
      ...
      <div className="mt-6 bg-gradient-to-r from-[#D77E6C] to-[#E09080] rounded-2xl p-6 md:p-8 text-white mb-8">
        <div className="max-w-2xl">
          <h1 className="text-2xl md:text-3xl font-bold mb-3">Развивайте навыки вместе с Tannur</h1>
      ...
      {activeTab === 'all' ? (
        <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {categories.map((category, index) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-gray-900 mb-1">{category.title}</p>
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-900">Рекомендуемые курсы</h2>
            <Link
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {courses.map((course) => (
              ...
              >
                ...
                {/* Контент */}
                <div className="p-4">
                  <span className="inline-block text-xs text-[#D77E6C] font-medium mb-2">{course.category}</span>
                  ...
                  {/* Псевдо-кнопка (без вложенных ссылок) */}
        ...
        <div className="bg-white rounded-2xl p-12 text-center">
          <div className="max-w-sm mx-auto">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Нет сохраненных курсов</h3>
            <p className="text-sm text-gray-500 mb-6">Сохраняйте интересные курсы, чтобы вернуться к ним позже</p>
            <button 
              onClick={() => setActiveTab('all')}
