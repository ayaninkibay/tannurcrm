...
import {
  ArrowLeft, Play, BookOpen, Clock, Award, FileDown
} from 'lucide-react';
...
const MOCK: Record<number, Course> = {
  1: { id: 1, title: 'Знакомство с продуктами Tannur', image: '/icons/edu_1.png', totalMin: 78, desc: 'Базовый курс…',
    lessons: [
  ...
  2: { id: 2, title: 'Маркетинговая стратегия 2.0', image: '/icons/edu_2.png', totalMin: 131, desc: 'Продвинутая стратегия…',
    lessons: [
...
export default function CoursePage() {
  const sp = useSearchParams();               // может быть null в типах проекта
  const router = useRouter();

  // --- читаем параметры БЕЗОПАСНО ---
  ...
  // смена урока — работаем через копию search params (если sp=null, берём пустые)
  ...
  return (
    <div className="p-2 md:p-6">
      ...
      <div className="mt-4 rounded-2xl p-6 md:p-8 text-white bg-[#D77E6C]">
        <h1 className="text-2xl md:text-3xl font-bold mb-1">Урок {activeLesson.id}: {activeLesson.title}</h1>
        <p className="text-white/90 text-sm md:text-base">Курс: <span className="font-medium">{course.title}</span></p>
        <div className="flex flex-wrap gap-4 md:gap-6 mt-5">
          <div className="flex items-center gap-2"><BookOpen className="w-5 h-5" /><span className="text-sm">Урок {activeLesson.id} из {course.lessons.length}</span></div>
          <div className="flex items-center gap-2"><Clock className="w-5 h-5" /><span className="text-sm">{minToHuman(activeLesson.durationMin)}</span></div>
          <div className="flex items-center gap-2"><Award className="w-5 h-5" /><span className="text-sm">Сертификат после курса</span></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
            ...
            <div className="relative w-full rounded-xl overflow-hidden bg-black">
              {!videoError ? (
                ...
                <div className="aspect-video flex items-center justify-center bg-gray-900 text-gray-200">
                  <div className="text-sm">Не удалось загрузить видео (заглушка)</div>
                </div>
              )}
            </div>
            <div className="mt-2 text-sm text-gray-700">
              <b>Смотрим:</b> {activeLesson.title} • {minToHuman(activeLesson.durationMin)}
            </div>
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
            <h3 className="text-base font-semibold text-gray-900 mb-2">О уроке</h3>
            <p className="text-sm text-gray-700 leading-6">{activeLesson.summary ?? course.desc}</p>
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            <h3 className="text-base font-semibold text-gray-900 mb-3">Материалы урока</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {['Методичка (PDF)', 'Чек-лист (DOCX)'].map((name, i) => (
                <a key={i} href="#" className="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-200 hover:shadow-sm">
        ...
        <aside className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
            <h3 className="text-sm font-semibold text-gray-900 mb-3">Уроки</h3>
          ...
          >
            <ArrowLeft className="w-4 h-4" /> К списку уроков
          </Link>
        </aside>
      </div>
    </div>
