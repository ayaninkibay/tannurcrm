...
export default function ProfileDealer() {
  ...
  useEffect(() => {
    const fetchDealerProfile = async () => {
      if (profile) setDealerProfile(profile);
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard
      .writeText(text)
      .then(() => alert(t('Ссылка скопирована!')))
      .catch(() => alert(t('Не удалось скопировать ссылку')));
  };
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#F3F3F3]">
        <div className="bg-white rounded-2xl p-8 shadow-lg">
          ...
          <p className="mt-4 text-gray-600">{t('Загрузка профиля дилера...')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#F3F3F3]">
        <div className="bg-white rounded-2xl p-8 shadow-lg text-center">
          <p className="text-red-500">{t('Ошибка: Профиль дилера не найден.')}</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 md:p-6 mb-4 sm:mb-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4 sm:mb-6">
          ...
          <div>
            <p className="text-xs sm:text-sm text-gray-600 ">
              {t('Команда {name}').replace('{name}', dealerProfile.first_name || t('Алина'))}
            </p>
            <p className="text-xl sm:text-2xl font-bold text-[#111]">
              {t('{count} человек').replace('{count}', String(teamCount))}
            </p>
          </div>

          {/* Avatar scheme */}
          <div className="relative w-full sm:w-[200px] h-[130px] mx-auto sm:mx-0">
            <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 130" preserveAspectRatio="xMidYMid meet">
            ...
            <img src="/icons/Users avatar 1.png" alt="user1" className="absolute top-[2px] left-[45%] sm:left-[90px] w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-[#DC7C67]" />
            <img src="/icons/Users avatar 2.png" alt="user2" className="absolute top-[35px] left-[30%] sm:left-[60px] w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-[#DC7C67]" />
            <img src="/icons/Users avatar 3.png" alt="user3" className="absolute top-[60px] left-[50%] sm:left-[100px] w-10 h-10 sm:w-11 sm:h-11 rounded-full border-2 border-white z-10 transform -translate-x-1/2 sm:translate-x-0" />
            <img src="/icons/Users avatar 5.png" alt="user4" className="absolute top-[79px] left-[25%] sm:left-[50px] w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-[#DC7C67]" />
            <img src="/icons/Users avatar 4.png" alt="user5" className="absolute top-[92px] left-[80%] sm:left-[160px] w-8 h-8 sm:w-9 sm:h-9 rounded-full border-2 border-[#DC7C67]" />
          </div>
        </div>
        ...
        <div className="w-full bg-[#DC7C67] text-white rounded-lg sm:rounded-xl p-2.5 sm:p-3">
          <div className="flex items-center gap-2 mb-1.5 sm:mb-2">
            ...
            <p className="text-[11px] sm:text-[12px] font-medium">{t('Ссылка для приглашения')}</p>
          </div>
          <div className="bg-white rounded-md px-2 py-1.5 sm:px-3 sm:py-2 flex justify-between items-center gap-2">
            ...
            <img onClick={handleCopy} src="/icons/Icon copy gray.png" className="w-3.5 h-3.5 sm:w-4 sm:h-4 cursor-pointer flex-shrink-0" alt="copy" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 md:p-6">
        <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-[#111]">
          {t('Награды от Tannur Cosmetics')}
        </h2>

        <p className="mt-2 text-xs sm:text-sm text-gray-500">
          {t('Описание наград (заглушка)')}
        </p>
        ...
        <div className="mt-4 sm:mt-6 grid grid-cols-2 gap-3 sm:gap-4">
          <div className="bg-gray-50 border border-gray-200 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 flex flex-col items-center text-center">
            <Image src="/icons/IconBulbPink.svg" alt="Партнер года" width={60} height={60} className="w-10 h-10 sm:w-12 sm:h-12 md:w-[60px] md:h-[60px] mb-2 sm:mb-3 md:mb-4" />
            <Image src="/icons/LogotipTannurOrange.svg" alt="Tannur logo" width={40} height={12} className="w-8 h-2.5 sm:w-10 sm:h-3 mb-1 sm:mb-2" />
            <p className="text-[10px] sm:text-xs md:text-sm text-[#111] leading-tight">
              {t('Партнер года {year}').replace('{year}', '2025')}<br />
              {t('II место')}
            </p>
          </div>

          <div className="bg-gray-50 border border-gray-200 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 flex flex-col items-center text-center">
            <Image src="/icons/IconballoonsPink.svg" alt="Участник Awards" width={60} height={60} className="w-10 h-10 sm:w-12 sm:h-12 md:w-[60px] md:h-[60px] mb-2 sm:mb-3 md:mb-4" />
            <Image src="/icons/LogotipTannurOrange.svg" alt="Tannur logo" width={40} height={12} className="w-8 h-2.5 sm:w-10 sm:h-3 mb-1 sm:mb-2" />
            <p className="text-[10px] sm:text-xs md:text-sm text-[#111] leading-tight">
              {t('Участник')}<br />
              {t('Tannur Awards {year}').replace('{year}', '2025')}
            </p>
          </div>
        </div>
      </div>
  ...
  const rightColumnContent = (
    <>
        ...
        <div className="relative pt-16 sm:pt-20 md:pt-28 lg:pt-36 p-4 sm:p-5 md:p-6 lg:p-8">
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0 ">
                <p className="text-base sm:text-lg font-semibold text-[#111] flex items-center gap-1 mt-5">
                  {(dealerProfile.first_name || t('Имя'))} {(dealerProfile.last_name || t('Фамилия'))}
                  <Image src="/icons/IconCheckMarkBlue.svg" alt="verify" width={14} height={14} className="flex-shrink-0" />
                </p>
                <p className="text-xs sm:text-sm text-gray-800">{t('Эксперт')}</p>
                <p className="text-xs sm:text-sm text-gray-500 truncate">{dealerProfile.email || t('N/A')}</p>
                <p className="text-xs sm:text-sm text-gray-500">{dealerProfile.phone || t('N/A')}</p>
              </div>
            </div>

            <div className="pt-3 border-t border-gray-200">
              <p className="text-xs sm:text-sm text-black">{t('Товарооборот')}</p>
          ...
          <div className="hidden md:flex md:items-start lg:items-center gap-4 md:gap-6">
            ...
            <div className="flex-1 flex flex-col justify-center gap-1 min-w-0 mt-5">
              ...
              <p className="text-sm md:text-base text-gray-800">{t('Эксперт')}</p>
              <p className="text-sm text-gray-500 truncate">{dealerProfile.email || t('N/A')}</p>
              <p className="text-sm text-gray-500">{dealerProfile.phone || t('N/A')}</p>
            </div>

            <div className="ml-auto flex-shrink-0">
              <p className="text-sm md:text-base text-black text-right">{t('Товарооборот')}</p>
      ...
      <div className="flex justify-between mt-4 sm:mt-6 items-center px-4 sm:px-6 md:px-8 text-[#111] text-lg sm:text-xl font-semibold mb-4 sm:mb-6">
        <span>{t('Лента')}</span>
        <Image src="/icons/IconSettingsBlack.svg" alt="settings" width={20} height={20} className="w-4 h-4 sm:w-5 sm:h-5" />
      </div>
      ...
      <div className="mt-4 sm:mt-6 grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        <DealerPost
          ...
          alt="Gallery image 1"
          title={t('Демо-заголовок поста')}
          description={t('Демо-описание поста')}
        />
        <DealerPost
  ...
  return (
    <>
      ...
      <div className="md:hidden">
        <MoreHeaderAD title={t('Tannur / Профиль дилера')} />
      ...
      <div className="hidden md:block p-6">
        <ThirdTemplate
          header={<MoreHeaderAD title={t('Tannur / Профиль дилера')} />}
