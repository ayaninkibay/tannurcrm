...
export default function CreateManagerPage() {
  ...
  return (
    <div className="p-2 md:p-6">
      <MoreHeaderAD title={t('Создать менеджера')} showBackButton={true} />
      ...
      <div className="mt-4 bg-white rounded-2xl border border-gray-100 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="block">
            <span className="text-sm text-gray-700">{t('Telegram')}</span>
            <input
              value={telegram}
          ...
          <label className="block">
            <span className="text-sm text-gray-700">{t('Регион')}</span>
            <input
              value={region}
              onChange={e => setRegion(e.target.value)}
              className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#DC7C67]"
              placeholder={t('Алматы')}
            />
          </label>

          <label className="block">
            <span className="text-sm text-gray-700">{t('Канал')}</span>
            <select
              value={channel}
            ...
            >
              ...
              <option>{t('Telegram')}</option>
              <option>{t('Instagram')}</option>
              <option>{t('Телефон')}</option>
            </select>
          </label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <label className="block">
              <span className="text-sm text-gray-700">{t('Комиссия, %')}</span>
              <input
                type="number"
                min={0}
                value={commissionPct}
          ...
          <label className="block md:col-span-2">
            ...
            <textarea
              ...
              rows={3}
              className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#DC7C67]"
              placeholder={t('Доп. условия, график…')}
            />
          </label>
        </div>
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  addTo(setTags, tagInput, () => setTagInput(''));
                }
              }}
              className="w-full md:w-1/2 rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#DC7C67]"
              placeholder={t('лидоген, ключевой и т.д.')}
            />
            <button
              type="button"
              onClick={() => addTo(setTags, tagInput, () => setTagInput(''))}
          ...
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  addTo(setResponsibilities, respInput, () => setRespInput(''));
                }
              }}
              className="w-full md:w-2/3 rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#DC7C67]"
              placeholder={t('Напр.: ежедневные обзвоны, отчёт по CRM…')}
            />
            <button
              type="button"
              onClick={() => addTo(setResponsibilities, respInput, () => setRespInput(''))}
              className="px-3 py-2 rounded-lg border text-sm hover:bg-gray-50"
            >
              {t('Добавить пункт')}
            </button>
            ...
            <ul className="mt-3 space-y-2">
              {responsibilities.map((o, i) => (
                <li key={i} className="flex items-start justify-between gap-3 bg-gray-50 rounded-lg px-3 py-2">
                  ...
                  <button
                    type="button"
                    onClick={() => removeFrom(setResponsibilities, i)}
        ...
        {/* направления/линейки */}
        <div>
          <div className="text-sm font-semibold text-gray-900 mb-2">{t('Линейки/направления')}</div>
          <div className="flex gap-2">
            <input
              value={prodInput}
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  addTo(setProducts, prodInput, () => setProdInput(''));
                }
              }}
              className="w-full md:w-2/3 rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#DC7C67]"
              placeholder={t('Напр.: Hair Care, Face Care…')}
            />
            <button
              type="button"
              onClick={() => addTo(setProducts, prodInput, () => setProdInput(''))}
              className="px-3 py-2 rounded-lg border text-sm hover:bg-gray-50"
            >
              {t('Добавить')}
            </button>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {products.map((p, i) => (
                <span key={i} className="inline-flex items-center gap-2 px-2.5 py-1.5 text-xs rounded-full bg-gray-100">
                  ...
                  <button
                    type="button"
                    onClick={() => removeFrom(setProducts, i)}
                    className="text-gray-500 hover:text-gray-700"
                  >
                    ×
                  </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <label className="inline-flex items-center gap-2 text-sm text-gray-800">
            <input type="checkbox" checked={canViewLeads} onChange={e => setCanViewLeads(e.target.checked)} /> {t('Доступ к лидам')}
          </label>
          <label className="inline-flex items-center gap-2 text-sm text-gray-800">
            <input type="checkbox" checked={canEditDeals} onChange={e => setCanEditDeals(e.target.checked)} /> {t('Редактирование сделок')}
          </label>
          <label className="inline-flex items-center gap-2 text-sm text-gray-800">
            <input type="checkbox" checked={canViewWarehouse} onChange={e => setCanViewWarehouse(e.target.checked)} /> {t('Склад / остатки')}
          </label>
          <label className="inline-flex items-center gap-2 text-sm text-gray-800">
            <input type="checkbox" checked={canViewFinance} onChange={e => setCanViewFinance(e.target.checked)} /> {t('Финансы / отчёты')}
          </label>
          <label className="inline-flex items-center gap-2 text-sm text-gray-800">
            <input type="checkbox" checked={canManageTeam} onChange={e => setCanManageTeam(e.target.checked)} /> {t('Управление командой')}
          </label>
          <label className="inline-flex items-center gap-2 text-sm text-gray-800">
