// src/components/components_admins_dashboard/PaymentHistoryTable.tsx
'use client';
...
const PaymentHistoryTable: React.FC<Props> = ({
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'к. товарооборот':
        return <Users className="h-3 w-3 md:h-4 md:w-4 text紫-600 text-purple-600" />;
      default:
    ...
    <div className="bg-white rounded-xl md:rounded-2xl border border-gray-200">
      <div className="p-4 md:p-6 border-b border-gray-100">
        <div className="flex flex-col gap-3">
          <h2 className="text-base md:text-lg font-semibold text-gray-900">{t('История выплат')}</h2>
              ...
              <tr key={payment.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600">{t(payment.type)}</span>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
          ...
          <div key={payment.id} className="p-4 hover:bg-gray-50">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="text-right">
                ...
                <span
                  className={`inline-flex px-1.5 py-0.5 text-[10px] font-medium rounded-full mt-1 ${getStatusColor(
                    payment.status
                  )}`}
                >
                  {t(payment.status)}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1.5">
                {getTypeIcon(payment.type)}
                <span className="text-xs text-gray-600">{t(payment.type)}</span>
              </div>
