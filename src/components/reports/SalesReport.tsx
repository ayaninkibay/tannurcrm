...
const MobileSaleCard = ({ sale, t }: { sale: Sale; t: (k: string) => string }) => (
    ...
    <div className="flex items-start justify-between mb-3">
      ...
      {sale.status === "Успешно" ? (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-50 border border-green-200 rounded-lg">
          ...
          <span className="text-xs font-medium text-green-700">{t("Успешно")}</span>
        </div>
      ) : (
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 border border-red-200 rounded-lg">
          <XCircle className="w-3 h-3 text-red-500" />
          <span className="text-xs font-medium text-red-700">{t("Отмена")}</span>
        </div>
      )}
    </div>
    ...
    <div className="bg-white rounded-xl md:rounded-2xl border border-gray-100 p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 text-sm">{t("Аналитика успешных продаж")}</p>
        </div>
      ...
      <div className="md:hidden">
        {sampleSales.map((sale, idx) => (
          <MobileSaleCard key={idx} sale={sale} t={t} />
        ))}
      </div>
            ...
            <tbody>
              {sampleSales.map((sale, idx) => (
                <tr key={idx} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <code className="bg-gray-100 px-2 py-1 rounded text-xs font-mono text-gray-700">{sale.id}</code>
                  </td>
                  <td className="py-4 px-4">
                    ...
                    <div className="flex justify-center">
                      {sale.status === "Успешно" ? (
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-green-50 border border-green-200 rounded-lg">
                          <CheckCircle2 className="w-3 h-3 text-green-500" />
                          <span className="text-sm font-medium text-green-700">{t("Успешно")}</span>
                        </div>
                      ) : (
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-red-50 border border-red-200 rounded-lg">
                          <XCircle className="w-3 h-3 text-red-500" />
                          <span className="text-sm font-medium text-red-700">{t("Отмена")}</span>
                        </div>
                      )}
                    </div>
                  </td>
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-gray-500">
        <p className="text-center sm:text-left">
          {t("Показано {n} записей").replace("{n}", String(sampleSales.length))}
        </p>
