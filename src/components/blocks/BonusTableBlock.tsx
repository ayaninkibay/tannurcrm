  ...
  return (
    <div className="flex-1 flex flex-col justify-start">
        ...
        <table className="min-w-full text-left text-[12px] md:text-sm text-gray-700">
          <thead className="bg-gray-100 text-gray-600 font-semibold sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 border-r border-gray-200">Процент</th>
              <th className="px-4 py-3">Ежемесячный доход</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => (
              <tr
                ...
                className={`border-t border-gray-200 ${
                  row.highlight ? 'text-green-600 font-semibold bg-green-50' : ''
                }`}
              >
                <td className="px-4 py-2 border-r border-gray-100">{row.turnover}</td>
                <td className="px-4 py-2 border-r border-gray-100">{row.percent}</td>
      ...
      <p className="text-xs text-gray-500 mt-4">
        Это ускорит рост командного дохода и позволит получить статус{' '}
        <strong className="text-black">БИЗНЕС ЭКСПЕРТА</strong>.
      </p>
    </div>
  )
}
