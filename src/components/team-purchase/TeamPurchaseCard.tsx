    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-xl font-semibold text-[#111] mb-2">
              {t('Заказ {n}').replace('{n}', String(purchase.order.order_number))}
            </h3>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{t('Организатор')}</p>
              </div>
            </div>
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex justify-between items-center mt-2">
            <span className="text-xs text-gray-500">
              {t('{p}% собрано').replace('{p}', String(Math.round(purchase.progress)))}
            </span>
            {purchase.daysLeft > 0 && (
              <span className="text-xs text-gray-500 flex items-center gap-1">
                <Clock className="w-3 h-3" />
                {t('Осталось {d} дн.').replace('{d}', String(purchase.daysLeft))}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <p className="text-xs text-gray-500 mb-1">{t('Участников')}</p>
          ...
          <div>
            <p className="text-xs text-gray-500 mb-1">{t('Товаров')}</p>
            <p className="text-sm font-semibold text-[#111]">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex -space-x-2">
            {purchase.participants.slice(0, 5).map((participant) => (
              <div
                key={participant.id}
          ...
          {purchase.daysLeft > 0 && purchase.daysLeft <= 3 && (
            <div className="px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-medium flex items-center gap-1">
              ...
              {t('Срочно! {d} дн.').replace('{d}', String(purchase.daysLeft))}
            </div>
          )}
        </div>
          ...
          {isParticipant && purchase.order.order_status === 'new' && (
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-gray-600">{t('Ваш вклад:')}</span>
