...
export default function BonusDisplay({ 
  ...
  const loadBonusData = async () => {
    try {
      ...
      // Подсчитываем суммы
      const personalBonus = bonuses?.filter(b => b.type === 'personal')
        .reduce((sum, b) => sum + (b.amount || 0), 0) || 0;
      
      const teamBonus = bonuses?.filter(b => b.type === 'team_difference')
        .reduce((sum, b) => sum + (b.amount || 0), 0) || 0;
      
      const pendingBonus = bonuses?.filter(b => b.status === 'pending_payment')
        .reduce((sum, b) => sum + (b.amount || 0), 0) || 0;
      
      const paidBonus = bonuses?.filter(b => b.status === 'paid')
        .reduce((sum, b) => sum + (b.amount || 0), 0) || 0;

      // Определяем следующий уровень
      const currentLevel = user?.personal_level || 5;
      const nextLevel = currentLevel < 7 ? 7 : currentLevel < 10 ? 10 : currentLevel < 15 ? 15 : 20;
      
      // Прогресс к следующему уровню (на основе товарооборота)
  ...
  const calculateMonthlyGrowth = (bonuses: any[]): number => {
    ...
    const thisMonthBonuses = bonuses.filter(b => 
      new Date(b.created_at) >= new Date(now.getFullYear(), now.getMonth(), 1)
  ...
  if (compact) {
    // Компактный вид для дашборда
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-[#111]">Мои бонусы</h3>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Ожидает выплаты</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-500">Рост</span>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-[#111] mb-4">Прогресс к следующему уровню</h3>
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-[#111]">Текущий уровень</p>
              <p className="text-sm text-gray-500">Личный бонус</p>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            <div>
              <p className="font-medium text-[#111] text-right">Следующий уровень</p>
              <p className="text-sm text-gray-500 text-right">Нужен товарооборот</p>
            </div>
        ...
        <p className="text-sm text-gray-500 mt-2">
          Прогресс: {bonusData.progressToNext.toFixed(0)}% до уровня {bonusData.nextLevel}%
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-[#111] mb-4">Источники дохода</h3>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-[#111]">Личные бонусы</p>
                  <p className="text-xs text-gray-500">С ваших покупок</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-[#111]">Командные бонусы</p>
                  <p className="text-xs text-gray-500">От вашей команды</p>
                </div>
              </div>
            ...
            <div className="pt-3 border-t border-gray-200">
              <div className="flex items-center justify-between">
                <p className="font-semibold text-[#111]">Итого заработано</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-[#111] mb-4">Статус выплат</h3>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-[#111]">Выплачено</p>
                  <p className="text-xs text-gray-500">Получено на счет</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-[#111]">Ожидает</p>
      ...
      {showHistory && (
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-[#111]">История начислений</h3>
            <div className="flex gap-2">
              {['week', 'month', 'all'].map(period => (
                ...
                >
                  {period === 'week' ? 'Неделя' : period === 'month' ? 'Месяц' : 'Все'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {bonusHistory.map(bonus => {
              ...
              return (
                <div key={bonus.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-xs font-medium ${
                      bonus.status === 'paid' ? 'text-green-600' : 
                      bonus.status === 'pending_payment' ? 'text-orange-600' : 
                      'text-gray-500'
                    }`}>
                      {bonus.status === 'paid' ? 'Выплачено' : 
                       bonus.status === 'pending_payment' ? 'Ожидает' : 
                       'Отменено'}
                    </p>
                  </div>
                </div>
