    ...
    <div className="w-full -mt-5 border-b border-gray-200 relative">
      <div className="h-[72px] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button onClick={() => setMenuOpen(true)} className="w-10 h-10 rounded-full flex items-center justify-center lg:hidden" aria-label="Open menu">
          ...
          <>
            ...
            >
              ...
              <div className="w-full text-[12px] text-gray-400 text-start mt-auto px-4">
                ...
                {t('Created by')} <span className="text-red-400 cursor-pointer" onClick={() => setShowInqoInfo(true)}>Inqo Technologies</span>.
              </div>
            </motion.div>
          </>
        )}
      ...
      <AnimatePresence>
        {showInqoInfo && (
          <motion.div className="fixed inset-0 z-[999] flex items-center justify-center backdrop-blur-sm bg-black/40 px-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowInqoInfo(false)}>
            <motion.div className="bg-white rounded-2xl p-8 text-center text-black shadow-xl" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ duration: 0.3 }} onClick={(e) => e.stopPropagation()}>
