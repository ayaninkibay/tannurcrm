...
const periodOptions = [
 { value: "all" as PeriodType, label: "За всё время" },
 { value: "last6" as PeriodType, label: "6 месяцев" },
 { value: "thisYear" as PeriodType, label: "Текущий год" },
 { value: "prevYear" as PeriodType, label: "Прошлый год" },
 { value: "custom" as PeriodType, label: "Выбрать период" },
];
...
export const TurnoverChart: React.FC<TurnoverChartProps> = ({
 ...
 const getPeriodLabel = () => {
   ...
   return periodOptions.find(o => o.value === period)?.label || 'За всё время';
 };
 ...
 return (
   <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 h-full">
               ...
               {periodOptions.map((option) => (
                 <button
                   ...
                   className={`w-full px-4 py-2 text-left hover:bg-gray-50 text-sm transition-colors flex items-center gap-2 ${
                     period === option.value && option.value !== 'custom' ? 'bg-gray-50 text-[#D77E6C] font-medium' : 'text-gray-700'
                   }`}
                 >
                   {option.value === 'custom' ? (
     ...
     {showDatePicker && (
       <>
         ...
         <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl z-50 p-6 w-full max-w-md">
           <h3 className="text-lg font-medium mb-4">Выберите период</h3>
     ...
     {/* Статистика - условное отображение */}
     {showStats && (
       <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
         ...
         <div className="bg-gray-50 rounded-xl px-4 py-3">
           <p className="text-xs text-gray-500 mb-1">Прирост</p>
           {isClient && (
             <p className={`text-xl md:text-2xl font-bold ${
               Number(growthRate) > 0 ? 'text-green-600' : Number(growthRate) < 0 ? 'text-red-600' : 'text-gray-900'
             }`}>
...
// Экспорт дополнительных вариантов для удобства использования
